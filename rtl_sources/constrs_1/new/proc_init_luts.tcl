set mem_file_path "/home/eightpins/RANC/mem"
set num_cores 5

proc proc_init_lut {current_core current_neuron neu_cons {num_luts 4} } {
	...
	for {set index 0} {$index < $num_luts} {incr index} {
		...
		set_property INIT $value [get_cells $current_cell]
...
for {set current_core 0} {$current_core < $num_cores} {incr current_core} {
	...
	while {[gets $neuron_con_file each_line] != -1} {
		proc_init_lut $current_core $line_numb $each_line; # init lut
		incr line_numb
	}
